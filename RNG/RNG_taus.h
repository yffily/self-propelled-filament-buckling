...
/*
Modified by Y. Fily (2012).
Added random_poisson.
Added get_state and set_state to save and reload the state of the RNG.
*/
...
public:
	...
	RNG_taus(const RNG_taus &other);     // copy constructor
	RNG_taus& operator = (const RNG_taus& other);
									                     // assignment operator
	...
	void get_state(unsigned long int & i1, unsigned long int & i2, unsigned long int & i3, double & x_, double & y_, bool & b);
	void set_state(unsigned long int i1, unsigned long int i2, unsigned long int i3, double x_, double y_, bool b);
	
	friend double random_normal(RNG_taus& rng);
	
private:
	unsigned long int s1, s2, s3;
	double x,y;           // for random_normal
	bool use_or_make;     // for random_normal
};
...
// poisson distribution P(n) = k^n * exp(-b) / n! with argument a=exp(-b)
unsigned int random_poisson ( RNG_taus & rng, const double & a ); 
